...
function middleware(filename, options, cb) {
  ...
  // Load helpers out of locals
  var contextHelpers = options._helpers;
  for( var index in options._helpers ) { 
    handlebars.helpers[ index ] = options._helpers[ index ];
  }
...
Instance.prototype.registerPartials = function (directory, done) {
  ...
  walk(directory, { followLinks: true }).on('file', function(root, stat, next) {
